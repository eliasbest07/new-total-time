...
interface MisionesCompactMockProps {
  misiones: Mision[];
...
const MisionCompactCard: React.FC<MisionCompactCardProps> = ({ mision, onClick }) => {
  const handleDragStart = (e: React.DragEvent) => {
    console.log('[DEMO] Drag started for mission:', mision.nombre);
    const misionData = {
      type: 'mision',
  ...
  const handleClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    console.log('[DEMO] MisionCompactCard clicked:', mision.nombre);
    if (onClick) {
      onClick();
    }
  };
...
export default function MisionesCompactMock({ misiones, onShowDetails }: MisionesCompactMockProps) {
      ...
      <div className="flex gap-2">
        {currentMisiones.map((mision) => (
          <MisionCompactCard
            ...
            mision={mision}
            onClick={() => {
              console.log('[DEMO] Misión seleccionada:', mision);
              if (onShowDetails) {
